...
SYN={"net_profit":["net_profit","net","pnl","profit"],"win_rate":["win_rate","win","winrate","wr","win_percent"],"max_drawdown":["max_drawdown","max_dd","dd","drawdown"],"trades":["trades","n_trades","count"]}
def pick(d,ks):
  for k in ks:
...
try:
  net=float(pick(m,SYN["net_profit"])); win=norm_ratio(pick(m,SYN["win_rate"])); dd=norm_ratio(pick(m,SYN["max_drawdown"])); n=int(pick(m,SYN["trades"]))
except Exception as e:
  print(f"[FAIL] metrics.json 不足/不正: {e}",file=sys.stderr); sys.exit(2)
ok=True
def j(label,val,cond,passed):
  global ok; print(f"{label}: {val} | gate: {cond} -> {'PASS' if passed else 'FAIL'}"); 
  globals()['ok']=ok and passed
